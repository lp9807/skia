...
public:
    GraphiteDawnVulkanWindowContext_win(HWND hwnd, const DisplayParams& params);
...
std::unique_ptr<WindowContext> MakeGraphiteDawnVulkanForWin(HWND hwnd,
                                                             const DisplayParams& params) {
    std::unique_ptr<WindowContext> ctx(new GraphiteDawnVulkanWindowContext_win(hwnd, params));
    if (!ctx->isValid()) {
        return nullptr;
    }
    return ctx;
